...
export function useFriendChat(friendId) {
  ...
  useEffect(() => {
    console.log("[E2EE] State - userId:", user?.id, "e2eeInitialized:", e2eeInitialized, "friendId:", friendId);
  ...
  const decryptMessageContent = useCallback(async (message) => {
    ...
    console.log("[E2EE] Checking message:", message.id, "isEncrypted:", !!encryptedData);
    ...
    try {
      ...
      if (!sharedKey) {
        return { ...message, content: "[Unable to decrypt - missing key]", decryptionFailed: true };
      }
      ...
      console.log("[E2EE] Decrypted successfully:", decryptedContent.substring(0, 20) + "...");
      return { ...message, content: decryptedContent, isEncrypted: true };
    } catch (err) {
      console.error("[E2EE] Decryption error:", err);
      return { ...message, content: "[Decryption failed]", decryptionFailed: true };
    }
  ...
  const decryptMessages = useCallback(async (msgs) => {
    ...
    const decrypted = await Promise.all(msgs.map(async (message) => {
      if (!message.content) return message;
      ...
      if (message.content.includes('encrypted')) {
        console.log("[E2EE] Message", message.id, "content preview:", message.content.substring(0, 80));
      ...
      try {
        ...
        console.error("[E2EE] Decryption error for message:", message.id, err.name, err.message);
        console.error("[E2EE] This usually means the message was encrypted with different keys (key mismatch)");
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      console.log("[E2EE] Re-decrypting messages, count:", currentMessages.length, "keyVersion:", keyVersion);
      
      const decrypted = await Promise.all(currentMessages.map(async (message) => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log("[E2EE] Initial fetch - setMessages called, last msg:", processedMessages[processedMessages.length - 1]?.content?.substring(0, 30));
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          console.log("[E2EE] Sender realtime - e2eeInitialized:", isE2EEReady, "content:", messageWithDetails.content?.substring(0, 50));
          
          // Always try to decrypt if E2EE is ready
          if (isE2EEReady) {
            const encryptedData = parseEncryptedContent(messageWithDetails.content);
            if (encryptedData) {
              try {
                ...
                if (sharedKey) {
                  ...
                  messageWithDetails = { ...messageWithDetails, content: decryptedContent, isEncrypted: true };
                  console.log("[E2EE] Sender realtime decrypted:", decryptedContent.substring(0, 30));
                }
              } catch (err) {
          ...
          setMessages((prev) => {
            if (prev.some((m) => m.id === payload.new.id)) {
              console.log("[E2EE] Sender realtime - message already exists, skipping:", payload.new.id);
              return prev;
            }
            console.log("[E2EE] Sender realtime - adding message to state:", messageWithDetails.id, "content:", messageWithDetails.content?.substring(0, 30));
      ...
      .on(
        ...
        async (payload) => {
          ...
          // Use ref to get latest e2eeInitialized value (avoid stale closure)
          const isE2EEReady = e2eeInitializedRef.current;
          console.log("[E2EE] Receiver realtime - e2eeInitialized:", isE2EEReady, "content:", messageWithDetails.content?.substring(0, 50));
            ...
            if (encryptedData) {
              try {
                ...
                if (sharedKey) {
                  ...
                  messageWithDetails = { ...messageWithDetails, content: decryptedContent, isEncrypted: true };
                  console.log("[E2EE] Receiver realtime decrypted:", decryptedContent.substring(0, 30));
                }
              } catch (err) {
          ...
          setMessages((prev) => {
            if (prev.some((m) => m.id === payload.new.id)) {
              console.log("[E2EE] Receiver realtime - message already exists, skipping:", payload.new.id);
              return prev;
            }
            console.log("[E2EE] Receiver realtime - adding message to state:", messageWithDetails.id, "content:", messageWithDetails.content?.substring(0, 30));
            return [...prev, messageWithDetails];
          });
      ...
      .on(
        ...
        (payload) => {
          console.log("[E2EE] UPDATE event received for message:", payload.new.id);
          ...
          setMessages((prev) =>
            prev.map((chat) => {
              if (chat.id === payload.new.id) {
                ...
                console.log("[E2EE] UPDATE - preserving content:", chat.content?.substring(0, 30));
  ...
  const sendMessage = async (content, attachments = []) => {
    ...
    try {
      ...
      if (e2eeEnabled && e2eeInitialized && messageContent) {
        ...
        if (sharedKey) {
          const { ciphertext, iv } = await encryptMessage(messageContent, sharedKey);
          messageContent = JSON.stringify(createEncryptedPayload(ciphertext, iv));
        }
      }
  ...
  return { messages, isLoading, sendMessage, messagesEndRef, isTyping, sendTyping, addReaction, uploadFile, e2eeEnabled };
}
